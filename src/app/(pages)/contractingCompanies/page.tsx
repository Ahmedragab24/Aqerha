...
import { EngineeringOfficesData2 } from "@/constants/cards/EngineeringOffices";
...
const ContractingCompaniesPage = () => {
  ...
  const getFilteredData = (city: string) => {
    if (city === "الكل") return EngineeringOfficesData2;
    return EngineeringOfficesData2.filter((item) => item.city === city);
  };
          ...
          return (
            ...
            >
              ...
              >
                {filteredData.map((item) => (
                  <EngineeringOfficesCard2
                    key={item.id}
                    EngineeringOffices={item}
                    path={`/contractingCompanies/${item.id}`}
                  />
                ))}
              </div>
              ...
              {filteredData.length === 0 && (
                <DataNotFount
                  title="لا توجد شركات مقاولات"
                  description={`لم يتم العثور على شركات مقاولات في ${city} حالياً`}
                />
              )}

...
const ExaminationRequestCard = ({ ExaminationRequest }: Props) => {
  const currentStepIndex = steps.findIndex(
    (s) => s.label === ExaminationRequest.examination_status
  );
    ...
    <Card className="w-full shadow-lg mx-auto">
      <CardContent className="p-4 sm:p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 sm:gap-0 mb-6">
          ...
          <div className="order-1 sm:order-2 text-right">
            <div className="font-semibold text-gray-800 text-base sm:text-lg">
              رقم الطلب: {ExaminationRequest.id}
            </div>
          </div>
          ...
          <div className="order-2 sm:order-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-600">
                {new Date(ExaminationRequest.created_at).toLocaleDateString()}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-6">
          {/* View Request Button */}
          <ViewRequestDetailsExaminationDialog
...
export default ExaminationRequestCard;
